...
\title{Understanding LoRA Rank Trade-offs in Diffusion Model Fine-Tuning}
\author{
  First Author\thanks{Department, Institution, City, Country. Email: author1@example.edu}
  \and
  Second Author\thanks{Department, Institution, City, Country. Email: author2@example.edu}
}
\date{}
...
\begin{abstract}
  % ============================================================
  Selecting the LoRA rank for diffusion model fine-tuning is a practical systems problem:
  higher ranks increase trainable capacity and adaptation cost, while lower ranks improve
  efficiency but may underfit. We present an empirical engineering study of LoRA rank
  trade-offs under controlled training budgets in diffusion fine-tuning. Our primary
  evaluation uses a DDPM U-Net backbone on CIFAR-10 with LoRA ranks \(\{2, 4, 8, 16, 32\}\)
  under fixed optimization settings, and reports parameter counts, runtime, GPU memory, and
  image quality measured by FID. We then strengthen the analysis with two targeted
  validations: (i) an extended-budget DDPM setting (ranks 4, 8, 16; 20 epochs), and (ii) a
  lightweight Tiny DiT validation (ranks 4, 8, 16; 10 epochs). All experiments use a
  local-folder \texttt{pytorch-fid} protocol with a reproducible CIFAR-10 test reference
  and 2000 generated evaluation samples per rank.
  
  Across controlled DDPM runs, moderate ranks provide the most favorable efficiency-quality
  trade-off: rank 4 attains the best FID (\num{124.1380}), rank 8 is competitive
  (\num{124.2136}), and higher ranks show diminishing returns despite larger trainable
  parameter budgets. In extended-budget DDPM validation, rank 16 improves only modestly
  (\(\approx\)0.68\% relative FID gain from 10 to 20 epochs), without overturning the
  moderate-rank trend. Tiny DiT validation exhibits a similar ordering, with moderate ranks
  again outperforming rank 16. These results support a practical, reproducible guideline:
  under constrained and fixed budgets, small-to-moderate LoRA ranks are often a strong first
  choice for diffusion fine-tuning.
  \end{abstract}
  ...
  Diffusion models have emerged as a compelling foundation for image generation, yet
  end-to-end fine-tuning of these models remains computationally demanding in practical
  settings. Parameter-efficient fine-tuning (PEFT) methods alleviate this burden by
  restricting gradient updates to a small subset of parameters rather than the full backbone.
  Among available PEFT approaches, Low-Rank Adaptation (LoRA) is particularly attractive for
  diffusion fine-tuning: it is straightforward to implement, integrates naturally with modern
  tooling, and exposes a single interpretable hyperparameter---rank $r$---that directly
  governs adaptation capacity \cite{hu2022lora}.
  
  In practice, however, selecting an appropriate LoRA rank remains a non-trivial decision.
  Very low ranks may restrict representational flexibility and limit the model's ability to
  adapt to new distributions, while unnecessarily large ranks increase trainable parameter
  counts, memory pressure, and runtime without commensurate quality improvements. Despite
  growing interest in diffusion model adaptation, existing literature largely emphasizes model
  quality and architectural scaling, offering limited controlled guidance on rank selection
  for reproducible fine-tuning pipelines \cite{ho2020ddpm, peebles2023dit}. This gap is
  especially consequential in resource-constrained settings where training budgets are fixed
  and experimental reproducibility is a hard requirement.
  
  This paper addresses that gap through a controlled empirical study of LoRA rank trade-offs
  in diffusion fine-tuning. We conduct a primary sweep over ranks $\{2, 4, 8, 16, 32\}$
  using a DDPM U-Net backbone on CIFAR-10, then assess trend stability through two focused
  validations: extended DDPM training at ranks $\{4, 8, 16\}$ over 20 epochs, and a second
  lightweight backbone (Tiny DiT) at ranks $\{4, 8, 16\}$ over 10 epochs. Quality is
  measured via FID using a reproducible local \texttt{pytorch-fid} protocol, alongside
  parameter efficiency, runtime, and memory metrics. Optimization settings are held fixed
  across all ranks to isolate the effect of rank under a controlled training budget, rather
  than to identify per-rank hyperparameter optima.
  ...
  implementations of models and noise schedulers across a wide range of architectures,
  enabling fair cross-study comparisons. Reproducible image-quality evaluation commonly
  relies on the Fr\'{e}chet Inception Distance (FID) \cite{heusel2017fid}, while CIFAR-10
  remains a widely used controlled benchmark that facilitates rapid and resource-efficient
  comparative studies \cite{krizhevsky2009cifar}. The present study builds directly on these
  foundations, employing a local \texttt{pytorch-fid} protocol with a fixed CIFAR-10 test
  reference to ensure evaluation consistency across all experimental tracks.
