...
def main():
    ...
    validation_path = logs_dir / "tiny_dit_validation.json"
    ...
    record_check("Tiny DiT config count", len(cfgs) == 3, f"count={len(cfgs)}", failures, checks)
    ...
    for cfg_path in cfgs:
        ...
        sample_dir = out_dir / "generated_samples"
        sample_pngs = sorted(sample_dir.glob("sample_*.png")) if sample_dir.is_dir() else []
        record_check(f"rank{rank} sample image exists", len(sample_pngs) >= 1, str(sample_dir), failures, checks)
        record_check(
        ...
        record_check(
            f"rank{rank} merged transformer exists",
            (out_dir / "transformer" / "diffusion_pytorch_model.safetensors").is_file(),
            str(out_dir / "transformer" / "diffusion_pytorch_model.safetensors"),
            failures,
            checks,
        )
    ...
    tiny_csv = metrics_dir / "tiny_dit_results.csv"
    fid_csv = metrics_dir / "tiny_dit_fid_results.csv"
    record_check("tiny_dit_results.csv exists", tiny_csv.is_file(), str(tiny_csv), failures, checks)
    record_check("tiny_dit_fid_results.csv exists", fid_csv.is_file(), str(fid_csv), failures, checks)
    record_check("tiny DiT FID rows complete for ranks 4/8/16", has_pass_fid_rows(fid_csv, required_ranks), str(fid_csv), failures, checks)

    expected_figs = ["tiny_dit_fid_vs_rank.png", "tiny_dit_runtime_vs_rank.png", "tiny_dit_training_loss_curves.png"]
    if (metrics_dir / "extended_budget_results.csv").is_file():
        expected_figs.append("ddpm_vs_tiny_dit_fid_rank4_8_16.png")
    for fig in expected_figs:
        record_check(f"figure {fig} exists", (fig_dir / fig).is_file(), str(fig_dir / fig), failures, checks)

    status = "PASS" if not failures else "FAIL"
    print(f"\nTINY DIT VALIDATION RESULT: {status}")
    if failures:
